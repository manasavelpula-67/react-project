...
function IceCreams(){
    ...
    const icecreamItems = useSelector((state) => state.products.iceCreams);
    const dispatch = useDispatch();
    const [filter, setFilter] = useState("all");
    
    let filteredItems;
    if (filter === "above") {
      filteredItems = icecreamItems.filter((item) => item.price >= 100);
    } else if (filter === "below") {
      filteredItems = icecreamItems.filter((item) => item.price < 100);
    } else {
      filteredItems = icecreamItems;
    }
    ...
    if (page >= 0 && page < totalPages) {
      ...
      return (
        <div className="container mt-5 text-center m-5 p-5">
          <h1 className="mb-3 text-success fst-italic">IceCream Flavours</h1>
          <p className="lead">Here you can browse different types of icecreams .</p>
          <div className="mb-4">
            <input
            type="text"
